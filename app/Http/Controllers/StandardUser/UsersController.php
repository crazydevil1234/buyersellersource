    ...
    public function submit_trade_info($r){
        ...
        if ($validator->fails())
        {
            ...
            $trade_join_arr_data = [
                ...
                'accepted_delivery_terms' => (count($r->delivery_terms)>0) ? implode(",",$r->delivery_terms) : null,
                'accepted_payment_currency' => (count($r->payment_currency)>0) ? implode(",",$r->payment_currency) : null,
                'accepted_payment_type' => (count($r->payment_type)>0) ? implode(",",$r->payment_type) : null,
                'language_spoken' => (count($r->language)>0) ? implode(",",$r->language) : null,
            ];
    ...
    public function submit_factory_info($r){
        ...
        if ($validator->fails())
        {
            ...
            $data = [
                ...
                'contact_manufacturing'                 =>(count($r->contact_manufacturing)>1) ? implode(",",$r->contact_manufacturing) : null,
