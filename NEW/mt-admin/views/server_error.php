<?php
$min = '5.6.04'; $max = '7.3.0'; if (version_compare(PHP_VERSION, $min, '<') || version_compare(PHP_VERSION, $max, '>=')) { $exception = new \Exception('PHP_VERSION_NOT_COMPATIBILITY'); } if (!isset($exception)) { return true; } $errorMessage = ''; switch($exception->getMessage()) { case 'PHP_VERSION_NOT_COMPATIBILITY': $errorMessage = 'Current PHP version is not supported. Control panel requirements are:<br/><strong>PHP version 5.6, 7.0, 7.1, 7.2.</strong><br/><br/>Please contact your hosting provider and kindly ask them to switch your PHP version up to the recommended version.'; break; case 'INSTALLATION_FOLDER_EXISTS': $errorMessage = 'To ensure operation of the control panel, the <strong>mt-install</strong> folder should be deleted. Please delete it from your server.'; break; case 'DATABASE_CONNECTION_ERROR': $errorMessage = 'Failed to connect to database. Please contact your hosting provider for additional help.'; break; default: $errorMessage = $exception->getMessage(); break; } $now = time(); ?>
<!DOCTYPE html>
<html ng-app="install">
<head>
    <meta charset="utf-8">
...
<?php
Moto\System\Log::critical('SERVER_ERROR : [ ' . $exception->getCode() . ' ] ' . $exception->getMessage()); die(); 