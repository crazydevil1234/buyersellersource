<?php
error_reporting(E_ALL); @ini_set('display_errors', 'off'); @ini_set('log_errors', 'on'); require_once __DIR__ . '/common.php'; if (Moto\System::isInstalled()) { echo Moto\System::getResponse()->redirect('./../'); exit; } $now = time(); $application = Moto\System::app('Installation.Application'); ?>
<!DOCTYPE html>
<html ng-app="install">
<head>
    <meta charset="utf-8">
...
<body class="install-app guest-app">
    ...
    <script type="text/javascript" data-cfasync="false">
        angular.module('application.config.value', ['ng']).constant('application.config.value', <?php echo json_encode($application->getEnvironment())?> );
    </script>
